...
def talk_to_rivia(message: str) -> dict:
    ...
    try:
        response = game_manager.simulate_ai_response(message.strip())
        
        # 检查是否游戏结束
        is_ended = "[END:" in response
        ending = None
        if is_ended:
            ending_match = re.search(r"\[END:(.*?)\]", response)
            if ending_match:
                ending = ending_match.group(1)
                response = response.replace(f"[END:{ending}]", "").strip()
...
def get_game_state() -> dict:
    ...
    if game_manager is None:
        return {
            ...
            "available_scenes": list(SCENE_DATA_FRAGMENTS.keys()),
            "scene_description": {
    ...
    return {
        "current_scene": game_manager.current_scene,
        "current_level": game_manager.current_level,
        "has_scene_data": bool(game_manager.current_scene_data),
        "available_scenes": list(SCENE_DATA_FRAGMENTS.keys()),
        "scene_description": {
            "乡村小镇": "一个宁静的小镇，或许能唤醒童年的记忆",
            "海边": "广阔的海洋，海浪声中藏着什么秘密",
...
def get_story_progress() -> dict:
    ...
    # 分析当前场景的对话选项
    prompt = game_manager._generate_full_prompt()
    level_marker = f"[关卡 {current_level} 对话]"
    
    dialog_options = []
    match_level = re.search(f"{re.escape(level_marker)}\\n(.*?)(?={re.escape('[关卡')}|\\Z)", prompt, re.DOTALL)
    if match_level:
        level_content = match_level.group(1)
        # 提取玩家对话选项
        player_dialogs = re.findall(r"\*   \*\*玩家：(.*?)\*\*", level_content)
        dialog_options = player_dialogs
    
    return {
        "current_scene": current_scene,
        "current_level": current_level,
        "suggested_responses": dialog_options,
        "story_status": "进行中" if current_scene != "无" else "未开始"
    }
...
def main():
    ...
    while True:
        try:
            ...
            ai_response = mcp_manager.simulate_ai_response(player_input)
